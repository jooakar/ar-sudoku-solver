...
export default function findHomographicTransform(
  ...
  const A = math.matrix([
    ...
    [size, size, 1, 0, 0, 0, -size * corners.bottomRight.x, -size * corners.bottomRight.x],
    [0, 0, 0, size, size, 1, -size * corners.bottomRight.y, -size * corners.bottomRight.y]])
  ...
  lambda.forEach(function(val, index) { vals[index] = val; });
